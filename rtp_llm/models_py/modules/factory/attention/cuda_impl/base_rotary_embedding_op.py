...
def _resolve_cos_sin_cache(
    ...
    (``std::call_once``) and returns it for every later call regardless of the
    config passed in. Models whose layers use different rope parameters would
    therefore all get whichever config asked first — MiMo V2.5 has theta 1e7 on
    its 9 global-attention layers and 1e4 on its 39 sliding-window layers, so the
    SWA layers would silently be positioned with the GA theta.

    Take the singleton only when it actually matches (``check_rope_cache`` plus a
    length check, which that helper does not cover), otherwise build a dedicated
    cache and memoize it per config.
    """
