...
#   2. o_proj is BF16 (listed in quantization_config.ignored_layers, no weight_scale_inv),
#      so once tagged as MiMo the base quantization wrapper lets it through and it takes
#      the plain BF16 path.
...
#
# Non-text weights (visual.* / audio_encoder.* / speech_embeddings.* / model.mtp.*) are
...
# ---------------------------------------------------------------------------
# The ckpt's fused QKV weight is slab-interleaved: the whole tensor is divided along rows
# into 4 slabs, each slab holding a complete [Q_shard | K_shard | V_shard], and each slab
# is FP8 block-quantized independently (block=[128,128]). The runtime kernel expects a
# section-major [Q|K|V] layout. At TP==4 (the ckpt's native slab count) every rank gets
...
SLAB_LAYOUT: Dict[int, Dict[str, int]] = {
    ...
    8: {  # SWA layers (kv_heads=8): each slab has Q=16heads*192, K=2heads*192, V=2heads*128
        "q_rows": 3072,
        "k_rows": 384,
        "v_rows": 256,
        "slab_rows": 3712,
...
def check_qkv_scale_rows(s: torch.Tensor, kv_heads: int) -> None:
    ...
    different layout fails here first."""
...
def process_mimo_qkv_scale(ts: List[torch.Tensor], kv_heads: int) -> torch.Tensor:
    """process_fn for loading the qkv scale: check total scale rows == slab_scale_rows * 4.

    value_scale has been moved to o_proj (_transpose_with_value_scale), so the qkv path
    does not handle it."""
    s = ts[0]
...
def _tp_bypass(load_config: LoadConfig) -> bool:
    """Same short-circuit condition as AtomicWeight._split: no split needed on a single
    device."""
    return (
...
# ---------------------------------------------------------------------------
# o_proj process_fn: transpose, then fold in value_scale
# ---------------------------------------------------------------------------
...
# wrapper (see model_weight.is_mimo_v25_weight and the exclusion logic in
# PerBlockFp8Weight.support); it plays the same role as DSV4's is_v4_weight.


class MiMoAttnAtomicWeight(AttnAtomicWeight):
    """MiMo qkv weight (unquantized path, as registered). For a quantized ckpt
    MiMoPerBlockFp8Weight takes over; the _split override here serves the BF16 ckpt case.
    """
    ...
    def _split(
        ...
        # the last dim. The per-layer kv head count comes from this weight's own AttnConfig
        # rather than the single global value in load_config.
        cfg = self.config
...
# ---------------------------------------------------------------------------
# MiMo-specific FP8 quantization wrapper (following the V4PerBlockFp8Weight precedent)
# ---------------------------------------------------------------------------
...
class MiMoPerBlockFp8Weight(PerBlockFp8Weight):
    ...
    - value_scale has been moved to o_proj (_transpose_with_value_scale), so the qkv path
      no longer involves any float-domain operation.
    ...
    def _split(
        ...
        # TP==4 assertion: the MiMo V2.5 FP8 ckpt is natively sliced into TP=4 slabs, and
        # slabs cannot be merged or re-split in the FP8 domain
        ...
        # Zero-conversion slice: each rank takes its own slab row range and emits FP8 as-is
