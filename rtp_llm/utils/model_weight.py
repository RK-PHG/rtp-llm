...
class W:
    ...
    # Attention sink bias (one scalar per Q head; used by MiMo V2.5 SWA layers)
    ...
    gpt_style_tp_strategy: Dict[str, Any] = {
        ...
        # qo_head_idx, so each rank must hold its own head slice, not the whole
        # tensor.
        attn_sink_bias: sp_0,
        attn_o_w: sp_0,
        attn_o_z: sp_0,
...
def is_mimo_v25_weight(src_weight_info) -> bool:
    ...
    (class attribute ``is_mimo_v25 = True``). MiMo uses standard W slot
    names, so quant-wrapping dispatch discriminates by instance marker
    instead of the name prefix used by DSv4 (`is_v4_weight`)."""
